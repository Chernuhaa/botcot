...
bot = telebot.TeleBot('5957810286:AAEZX4jW6GZdTyG_FtJ_P8LQf8vZe5YVPiI')  # сюда нужно вписать ваш API key от официального бота @BotFather
...
def start(message):
    ...
    btn1 = types.KeyboardButton('Задай свой вопрос')
    markup.add(btn1)  # и добавляем их к нашей разметки
...
def mess(message):
    ...
    elif get_message_bot in ('задай свой вопрос', 'задать вопрос еще раз'):
        s = ['конечно!', 'думаю, ничего не выйдет(', 'попробуй и узнаешь', 'ты действительно хочешь узнать это?', 'да!', 'нет😢', 'хохохоо  no😬', 'yeeees🥳', 'по-моему это отличная мысль!']
        if get_message_bot == 'задай свой вопрос':
            bot.send_message(message.chat.id, f'Ты можешь задать любой вопрос,который тебя волнует! Но есть одно условие: задавай вопросы, на которые можно ответить да/нет. Чтож, давай заглянем в будущее!✨')
        elif get_message_bot == 'задать вопрос еще раз':
            x = ['Понравилось? Тогда продолжим!','Да ты настырный...Ладно, давай еще']
            text_message = random.choice(x)
            bot.send_message(message.chat.id, text_message)
        @bot.message_handler(content_types=['text'])
        def message_input(message):
            answer = s[random.randint(0, len(s)-1)]
            markup = types.ReplyKeyboardMarkup(resize_keyboard=True, row_width=2)
            btn1 = types.KeyboardButton('Задать вопрос еще раз')
            markup.add(btn1)
            f = open('photo/video.mp4', 'rb')
